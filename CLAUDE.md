...
Flask app: enter USCF ID + DOB (bulk supported), see how many months, games, and what age a
player took to reach a configurable set of rating milestones, plus cumulative score % at each.
Primary source is the US Chess JSON API; falls back to HTML scraping (only ~Oct 2025) if the API
fails. Data is cached in SQLite so we don't hammer USCF; cached scrapes auto-refresh once they
...
(drives the charts' "achieved …" hover). FIDE `score_pct` is real — reconstructed from per-period
W/D/L (`a_indv_calculation.php`), not `None`.
...
`session["milestones_uscf"|"milestones_fide"]` chosen by `session["source"]`, falling back to the
matching default. Passed explicitly into `compute_record` / `scrape_*(..., milestones=...)` — the
