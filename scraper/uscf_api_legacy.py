"""LEGACY USCF JSON API client — a verbatim snapshot of the pre-2026-07 client,
kept so we can revert instantly. This is the OLD behavior: it filters to
`RatingSource="R"` sections whose `ratingSystem == "R"`, which silently drops a
player's no-affiliate FIDE/foreign events (they carry an `R` rating record but a
`G`/`A`/`F` section system) — e.g. it collapses Caruana's 2008–2014 climb into a
single jump. The active client (`scraper/uscf_api.py`) fixes that.

Selected at runtime by `scraper.core.fetch_history` when `config.USCF_INCLUDE_FOREIGN`
is false (set env `USCF_INCLUDE_FOREIGN=0` to fall back to this).

See specs/006-uscf-api-source/plan.md.
"""
import logging
...
from scraper.uscf_api import ApiUnavailable  # share the class so `except` still matches
